...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // ===================================================================
  // PR #151-debug — TRUE component breakdown using loadTaxInputsForMonth.
  // PR #149 reconstructed components from scratch and got the wrong
  // tables/buckets (recon_orders.raw_json instead of recon_line_items,
  // missing recognized_at, missing gift-card filter on refunds, missing
  // non-POS-entity→0 funneling). This endpoint instead runs the SAME
  // loader the live tax endpoints use, then sums each of the 5 inputs
  // for one entity_id BEFORE handing them to aggregateByEntity. So the
  // formula matches production by construction; any delta vs by-store
  // is real.
  //
  // GET /api/recon/finance/debug/tax-component-truth/:month?entity_id=N
  // ===================================================================
  app.get("/api/recon/finance/debug/tax-component-truth/:month", authMiddleware, requirePermission("payroll.view"), async (req, res) => {
    const month = String(req.params.month);
    if (!/^\d{4}-\d{2}$/.test(month)) {
      return res.status(400).json({ message: "Month must be YYYY-MM" });
    }
    ...
    try {
      const { sqlite } = require("./storage");
      const { aggregateByEntity } = require("./shopify-tax-aggregation");

      // Use the PRODUCTION loader. Same inputs the live by-entity endpoint uses.
      const { inputs, refunds, shippingTaxForward, shippingTaxRefunds, unverifiedReturnTax, entityNames } =
        loadTaxInputsForMonth(month);

      // Sum each input bucket scoped to entity_id (post-attribution).
      let fwdLineCents = 0, fwdLineGiftSkipped = 0, fwdLineRows = 0;
      for (const { line } of inputs) {
        if (line.entity_id !== entityId) continue;
        if (line.is_gift_card) { fwdLineGiftSkipped++; continue; }
        fwdLineRows++;
        for (const tl of line.tax_lines) fwdLineCents += Math.round(Number(tl.price || 0) * 100);
      }

      let itemRefundCents = 0, itemRefundRows = 0;
      for (const r of refunds) {
        if (r.entity_id !== entityId) continue;
        itemRefundRows++;
        itemRefundCents += Math.round(Number(r.refund_tax || 0) * 100);
      }

      let fwdShipCents = 0, fwdShipRows = 0;
      for (const st of shippingTaxForward) {
        if (st.entity_id !== entityId) continue;
        fwdShipRows++;
        for (const tl of st.tax_lines) fwdShipCents += Math.round(Number(tl.price || 0) * 100);
      }

      let shipRefundCents = 0, shipRefundRows = 0;
      for (const sr of shippingTaxRefunds) {
        if (sr.entity_id !== entityId) continue;
        shipRefundRows++;
        shipRefundCents += Math.round(Number(sr.refund_tax || 0) * 100);
      }

      let unvCents = 0, unvRows = 0;
      for (const u of unverifiedReturnTax) {
        if (u.entity_id !== entityId) continue;
        unvRows++;
        unvCents += Number(u.tax_delta_cents || 0);
      }

      // Production aggregate for this entity—ground truth for tax_collected_gross.
      const summaries = aggregateByEntity(inputs, entityNames, refunds, shippingTaxForward, shippingTaxRefunds, unverifiedReturnTax);
      const entSummary = summaries.find((s: any) => s.entity_id === entityId) || null;

      // Local formula—should match aggregateByEntity if attribution is consistent.
      const formulaCents = fwdLineCents + fwdShipCents - itemRefundCents - shipRefundCents - unvCents;

      // by-store rollup using the SAME CTE the by-store endpoint uses.
      const salesByActionLine = sqlite.prepare(`
        WITH attributed AS (
          SELECT s.action_type, s.line_type, s.total_tax,
            COALESCE(
      ...
      // The lone UPDATE row in Dec is what we most want to see—pull its detail.
      const updateRows = sqlite.prepare(`
        SELECT s.order_id, s.line_item_id, s.action_type, s.line_type,
               s.total_tax, s.total_amount, s.pos_location_id, s.happened_at
          FROM recon_shopify_sales s
         WHERE s.happened_month = ?
      ...
      res.json({
        ...
        entity_id: entityId,
        components: {
          fwd_line_tax_dollars:          fmt(fwdLineCents),
          fwd_ship_tax_dollars:          fmt(fwdShipCents),
          item_refund_tax_dollars:       fmt(itemRefundCents),
          ship_refund_tax_dollars:       fmt(shipRefundCents),
          unverified_return_tax_dollars: fmt(unvCents),
        },
        component_counts: {
          fwd_line_rows:    fwdLineRows,
          fwd_line_gift_skipped: fwdLineGiftSkipped,
          item_refund_rows: itemRefundRows,
          fwd_ship_rows:    fwdShipRows,
          ship_refund_rows: shipRefundRows,
          unv_rows:         unvRows,
        },
        formula_local: {
          description: "fwd_line + fwd_ship - item_refund - ship_refund - unverified",
          tax_collected_gross_dollars: fmt(formulaCents),
        },
        aggregate_by_entity: entSummary ? {
          tax_collected_gross: entSummary.tax_collected_gross,
          pos_tax: entSummary.pos_split.tax,
          allocated_tax: entSummary.allocated_split.tax,
          marketplace_tax: entSummary.marketplace_tax_collected,
          tax_owed: entSummary.tax_owed,
        } : null,
        by_store: {
          taxes_dollars: fmt(bsTaxCents),
          breakdown_by_action_line: (salesByActionLine as any[]).map((r) => ({
          ...
          update_rows_in_month: (updateRows as any[]).map((r) => ({
            order_id: r.order_id,
        ...
        delta_dollars: entSummary
          ? (Math.round(parseFloat(entSummary.tax_collected_gross) * 100) - bsTaxCents) / 100
          : null,
        note: "PR #151-debug — sources from loadTaxInputsForMonth, identical to production by-entity inputs. If aggregate_by_entity.tax_collected_gross ≠ formula_local, attribution/marketplace flags differ. delta_dollars is the real entity-vs-store gap.",
        build_id: "pr151-debug-tax-component-truth",
      });
    } catch (e: any) {
      res.status(500).json({ message: "tax-component-truth failed", error: String(e?.message || e) });
    }
  });
